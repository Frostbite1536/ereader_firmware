...
// See FirmwareFlasher.h for provenance. Storage: SDCardManager/SdFat.
...
// Stream `length` bytes from `file`, feeding the XOR-checksum and SHA256
// accumulators, so the whole image verifies without holding it in RAM.
Result feedHashAndChecksum(FsFile& file, size_t length, uint8_t* xorAccum, mbedtls_sha256_context* sha, uint8_t* buf) {
...
const char* resultName(Result r) {
  switch (r) {
    ...
    case Result::OTADATA_FAIL: return "OTADATA_FAIL";
  }
  return "?";
}

Result validateImageFile(const char* sdPath, size_t partitionSize) {
  FsFile file = SdMan.open(sdPath, O_RDONLY);
  if (!file) {
    Serial.printf("[FLASH] validate: open failed: %s\n", sdPath);
  ...
  if (header[0] != ESP_IMAGE_MAGIC) {
    Serial.printf("[FLASH] validate: bad magic 0x%02X\n", header[0]);
    file.close();
  ...
  // pad_end is the 16-byte-aligned offset; the checksum byte sits at pad_end - 1.
  ...
  if ((xorAccum & 0xFF) != storedChecksum) {
    Serial.printf("[FLASH] validate: checksum mismatch 0x%02X != 0x%02X\n", xorAccum, storedChecksum);
    mbedtls_sha256_free(&shaCtx);
    file.close();
  ...
  if (hashAppended) {
    ...
    if (std::memcmp(computed, stored, SHA_TRAILER) != 0) {
      Serial.println("[FLASH] validate: SHA256 mismatch");
      mbedtls_sha256_free(&shaCtx);
      file.close();
...
Result flashFromSdPath(const char* sdPath, ProgressCb onProgress, void* ctx, bool alreadyValidated) {
  ...
  if (!dest) {
    Serial.println("[FLASH] no next-update partition");
  ...
  if (!alreadyValidated) {
    ...
    if (validateRes != Result::OK) {
      Serial.printf("[FLASH] image validation failed: %s\n", resultName(validateRes));
      return validateRes;
    }
  }
  ...
  FsFile file = SdMan.open(sdPath, O_RDONLY);
  if (!file) {
    Serial.printf("[FLASH] open failed: %s\n", sdPath);
    return Result::OPEN_FAIL;
  }

  const size_t firmwareSize = file.fileSize();
  Serial.printf("[FLASH] src=%s size=%u dest=%s @0x%x\n", sdPath, static_cast<unsigned>(firmwareSize), dest->label,
  ...
  // Interleave erase + write so progress advances smoothly.
  size_t streamPos = 0;
  size_t erasedUpto = 0;
  ...
  if (!ota_boot::switchTo(dest)) return Result::OTADATA_FAIL;
  return Result::OK;
}
